...
export default function ListingForm() {
  ...
  const { user, isAuthenticated, getValidToken, logout } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 202) {
        ...
        alert(result.message || "Listing submitted successfully and is under moderation!");
        router.push("/myposts");
      } else {
        const result = await response.json();
        ...
        if (response.status === 401 || response.status === 403) {
          ...
          alert("Error: " + (result.error || result.message || `Server returned status ${response.status}`));
        }
      }
    } catch (err) {
      ...
      if (err instanceof TypeError && err.message.includes('fetch')) {
        ...
        alert("Something went wrong while submitting the post. Please try again.");
      }
    } finally {
          ...
          <div className="mb-6">
            ...
            <select
              ...
              className={`mt-1 block w-full border ${errors.category ? "border-red-500" : "border-gray-300"
                } rounded-md shadow-sm focus:border-[#f76902] focus:ring-[#f76902] ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
                      ...
                      {["Non-smoker", "Pet-friendly", "Cooking-enthusiast"].map((pref) => (
                        <button
                          ...
                          className={`px-3 py-1 rounded-full border ${selectedPreferences.includes(pref)
                            ...
                            } ${isSubmitting ? 'opacity-50 cursor-not-allowed' : 'hover:bg-[#db5d00] hover:text-white'}`}
                        >
                          {pref}
                        </button>
                      ))}
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mt-1">
                      <input
                        ...
                        disabled={isSubmitting}
                        className={`block w-full border-gray-300 rounded-md shadow-sm focus:border-[#f76902] focus:ring-[#f76902] ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
                      />
                      <button
                        type="button"
                        onClick={handleAddCustomPreference}
                        disabled={isSubmitting}
                        className={`px-3 py-1 bg-[#f76902] text-white rounded-md hover:bg-[#db5d00] ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
                      >
                        Add
                      </button>
                    </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`mt-1 block w-full text-sm text-gray-700 
                                ...
                                hover:file:bg-[#eee] ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
                    />
                  </div>
                </>
              )}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`mt-1 block w-full border ${errors.pickupLocation ? "border-red-500" : "border-gray-300"
                        } rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-2 ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`mt-1 block w-full border ${errors.rideDate ? "border-red-500" : "border-gray-300"
                        } rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
                    />
          ...
          <button
            ...
            className={`mt-6 w-full py-2 px-4 rounded-md transition-colors ${
              isSubmitting
                ...
                : "bg-[#f76902] text-white hover:bg-[#db5d00]"
            }`}
          >
