...
export default function MyFeedPage() {
  ...
  const [userInfoCache, setUserInfoCache] = useState<Record<string, User>>({});
  ...
  const handleSemanticSearch = async () => {
    ...
    try {
      ...
      const data = await response.json();
      const fetchedPosts: Post[] = (data as Post[]) || [];
  ...
  useEffect(() => {
    ...
    fetchPosts();
  }, [page, limit, searchQuery, category, sortOrder, isSemanticSearch, getValidToken]);
  ...
  useEffect(() => {
    ...
    console.log("Sample posts with images:", posts.slice(0, 3).map(post => ({
      postId: post._id,
  ...
  const getImageSrc = (post: Post) => {
    if (post.images && post.images.length > 0) {
      ...
      // Prefer medium > large > thumbnails
      if (urls.medium) imageUrl = urls.medium;
      else if (urls.large) imageUrl = urls.large;
      else if (urls.thumbnails) imageUrl = urls.thumbnails;
      
      // If we have an imageUrl, construct full URL with API_URL
      if (imageUrl) {
    ...
    // Fallback: default category images from /public (these are local assets)
    return DEFAULT_IMAGES[post.category] || "/default.png";
  };
              ...
              {selectedPost.images && selectedPost.images.length > 0 && (
                <div className="flex flex-wrap justify-center gap-4 mt-4">
                  {selectedPost.images.map((image, idx) => {
                    const imageUrl = image.urls.large || image.urls.medium || image.urls.thumbnails;
                    const fullImageUrl = imageUrl ? `${API_URL}/${imageUrl.startsWith('/') ? imageUrl.substring(1) : imageUrl}` : '';
                    
                    return (
                      <img
                        key={idx}
                        src={fullImageUrl || DEFAULT_IMAGES[selectedPost.category] || '/default.png'}
                        alt={`Image ${idx + 1}`}
